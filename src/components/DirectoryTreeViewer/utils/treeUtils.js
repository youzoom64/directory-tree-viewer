// 制限付きディレクトリの定義
const RESTRICTED_DIRECTORIES = new Set(["lib64"]);

// エラーが起きやすいディレクトリの定義
const SKIP_DIRECTORIES = new Set([".git", "__pycache__", ".idea", ".vscode"]);
...
const generateTreeText = (
  ...
  if (node.type === "file") {
    if (!showFiles) return "";
    if (fileFilter && !node.name.includes(fileFilter)) return "";

    // 拡張子フィルタリング
    // 拡張子フィルタリング（選択したものを非表示）
    if (selectedExtensions.size > 0) {
      const ext = "." + node.name.split(".").pop();
      if (selectedExtensions.has(ext)) return "";
    }
  }
  // インデントと階層マーカーの生成
  if (level === 0) {
  ...
  try {
    ...
    const children = await Promise.all(
      entries.map(async (childEntry) => {
        try {
          return await readDirectory(childEntry, {
            onFileCount,
            onFolderCount,
          });
        } catch (error) {
